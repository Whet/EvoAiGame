...
import pareto.ParetoEvolver.Individual;
import utils.Maths;
import ai.Ai;

public class ParetoGameSimulator {
	// 20 turns a second
	// 1 min of gameplay
	...
	public static int runGame(WorldMap map, Individual individual1, Individual individual2) {
		...
		if(subGameOutcome == 1) {
			individual1.victories.put(individual2, true);
			individual2.victories.put(individual1, false);
		}
		else if(subGameOutcome == -1) {
			individual1.victories.put(individual2, false);
			individual2.victories.put(individual1, true);
		}
	...
	private static int scoreCalculation(int ai1FlagCaptures,
			double ai1DamageScore, int ai2FlagCaptures, double ai2DamageScore) {
